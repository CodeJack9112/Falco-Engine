...
void DLight::setAttenuation(float c, float b, float a)
{
    ...
    if (c == oldAttenuationConstant &&
        b == oldAttenuationLinear &&
        a == oldAttenuationQuadric &&
        oldRadius == outerRadius &&
        oldSpotlightFalloff == mParentLight->getSpotlightFalloff() &&
        oldSpotlightInnerAngle == mParentLight->getSpotlightInnerAngle() &&
        oldSpotlightOuterAngle == mParentLight->getSpotlightOuterAngle())
        return;
    
    oldAttenuationConstant = c;
    oldAttenuationLinear = b;
    oldAttenuationQuadric = a;
    oldRadius = outerRadius;
...
{
    return mGenerator->getMaterial(mPermutation, mParentLight->getName());
}
//-----------------------------------------------------------------------
...
void DLight::updateFromParent()
{
    ...
    if (getCastShadows() && mParentLight->getMode() != Light::LightMode::LM_BAKED && SceneManager::shadowsEnabled)
    {
    ...
    else
    {
        ...
        if (oldCastShadows)
        {
            oldCastShadows = false;
            mParentLight->deleteShadowMaps();
        }
    }
}
//-----------------------------------------------------------------------
...
{
	return mParentLight->getCastShadows();
}
//-----------------------------------------------------------------------
void DLight::updateFromCamera(Ogre::Camera* camera)
{
    ...
    for (unsigned short i=0; i<tech->getNumPasses(); i++) 
    {
        ...
        if (params->_findNamedConstantDefinition("shadowCamPos"))
            params->setNamedConstant("shadowCamPos", mParentLight->getDerivedPosition());

        if (params->_findNamedConstantDefinition("shadowFarClip"))
            params->setNamedConstant("shadowFarClip", camera->getFarClipDistance());

        if (params->_findNamedConstantDefinition("shadowBias"))
            params->setNamedConstant("shadowBias", mParentLight->getShadowBias());
    }
}
