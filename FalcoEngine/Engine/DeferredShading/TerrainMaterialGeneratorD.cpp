...
#include "../Ogre/source/OgreMain/include/OgreHighLevelGpuProgram.h"
#include "../Ogre/source/OgreMain/include/OgreLogManager.h"

namespace Ogre
{
	int TerrainMaterialGeneratorD::MAX_LAYERS = 16;
	//---------------------------------------------------------------------
	...
	uint8 TerrainMaterialGeneratorD::SM2Profile::getMaxLayers(const Terrain* terrain) const
	{
		...
		uint8 freeTextureUnits = MAX_LAYERS;
		// lightmap
		--freeTextureUnits;
		// normalmap
		--freeTextureUnits;
	...
	MaterialPtr TerrainMaterialGeneratorD::SM2Profile::generate(const Terrain* terrain)
	{
		...
		if (mat.isNull())
		{
			...
			mat = matMgr.getByName(matName, "Assets");
			if (mat.isNull())
			{
				mat = matMgr.create(matName, "Assets");
			}
		}
		// clear everything
		...
		const SM2Profile* prof, const Terrain* terrain, TechniqueType tt, const HighLevelGpuProgramPtr& prog)
	{
		...
		params->setNamedAutoConstant("cNearDistance", Ogre::GpuProgramParameters::ACT_NEAR_CLIP_DISTANCE);
		params->setNamedAutoConstant("worldViewMatrix", GpuProgramParameters::ACT_WORLDVIEW_MATRIX); // tout sauf Z : VIEW_MATRIX
	}
	//---------------------------------------------------------------------
			...
			const SM2Profile* prof, const Terrain* terrain, TechniqueType tt)
	{
		...
		HighLevelGpuProgramPtr ret = mgr.getByName(progName, "Assets");
		if (ret.isNull())
		{
			ret = mgr.createProgram(progName, "Assets",
		...
		ret->setParameter("profiles", "glslv");
			...
			const SM2Profile* prof, const Terrain* terrain, TechniqueType tt)
	{
		...
		ret->setParameter("profiles", "glslf");
		...
		const SM2Profile* prof, const Terrain* terrain, TechniqueType tt, StringUtil::StrStreamType& outStream)
	{
		...
		outStream <<
			...
			"	oPosObj = mul(worldViewMatrix, pos);\n"
			"	oViewPos = mul(worldViewMatrix, pos);\n"
			"	oWorldPos = mul(worldViewProjMatrix, pos);\n";
		...
		const SM2Profile* prof, const Terrain* terrain, TechniqueType tt, StringUtil::StrStreamType& outStream)
	{
		...
		outStream <<
			...
			" out float4 oColor4 : COLOR4,\n"
			" out float4 oColor5 : COLOR5,\n"
			" out float4 oColor6 : COLOR6,\n"
			" out float4 oColor7 : COLOR7,\n"
			" in float4 oViewPos,\n"
			" in float4 oWorldPos,\n"
			//" out float4 oColor4 : COLOR4,\n"
			"uniform float cFarDistance,\n"
			"uniform float cNearDistance,\n"
			"uniform float4x4 worldViewMatrix,\n";
		...
		if (currentSamplerIdx > MAX_LAYERS)
		{
			OGRE_EXCEPT(Exception::ERR_INVALIDPARAMS,
		...
		outStream <<
			...
			"   oColor2 = float4(1,1,1,0);\n"
			"   oColor3 = float4(1,1,1,1);\n";
			"   oColor4 = float4(0,0,0,0);\n";
			"   oColor5 = float4(0,0,0,1);\n";
			"   oColor7 = float4(0,0,0,0);\n";
		...
		else
		{
			...
			// are provided by the "normal" texture, and not in vertex info. We multiply it by the worldViewMatrix
			// so the normals are in eye-space instead of object-space, as required by deferred shading.
			outStream << "	normal = mul(worldViewMatrix, float4(normal,0)).xyz;" << std::endl;
			outStream << "	tangent = mul(worldViewMatrix, float4(tangent,0)).xyz;" << std::endl;
		...
		const SM2Profile* prof, const Terrain* terrain, TechniqueType tt, StringUtil::StrStreamType& outStream)
	{
		...
		// Final return
		outStream << "  oColor1 = float4(normalize(mul(oColor1.rgb, TBN)), length(oViewPos) / cFarDistance);\n";
		outStream << "	oColor2 = oColor0; oColor2.a = 0.65; \n";
		outStream << "	oColor6.rgb = oViewPos.xyz;\n";
		outStream << "}\n";
	}
	//---------------------------------------------------------------------
}