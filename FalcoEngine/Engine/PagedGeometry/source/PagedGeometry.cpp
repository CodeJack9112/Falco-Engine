...
//-------------------------------------------------------------------------------------
PagedGeometry::PagedGeometry(Camera* cam, SceneManager* mgr, String name, const Real pageSize, Ogre::RenderQueueGroupID queue) :
m_nRenderQueue(queue)
{
	...
	if (cam)
	{
		...
		rootNode = sceneMgr->getRootSceneNode()->createChildSceneNode("PagedGeometry_root_" + name);	//Create PagedGeometry's root node
		#else
...
void PagedGeometry::setPageSize(Real size)
{
	if (!managerList.empty())
		OGRE_EXCEPT(Ogre::Exception::ERR_RENDERINGAPI_ERROR, "PagedGeometry::setPageSize() cannot be called after detail levels have been added. Call removeDetailLevels() first.", "PagedGeometry::setPageSize()");
...
{
	if (!managerList.empty())
		OGRE_EXCEPT(Ogre::Exception::ERR_RENDERINGAPI_ERROR, "PagedGeometry::setInfinite() cannot be called after detail levels have been added. Call removeDetailLevels() first.", "PagedGeometry::setInfinite()");
...
{
	if (!managerList.empty())
		OGRE_EXCEPT(Ogre::Exception::ERR_RENDERINGAPI_ERROR, "PagedGeometry::setBounds() cannot be called after detail levels have been added. Call removeDetailLevels() first.", "PagedGeometry::setBounds()");
