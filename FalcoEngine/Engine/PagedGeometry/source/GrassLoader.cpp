...
GrassLayer* GrassLoader::addLayer()
{
	GrassLayer* layer = new GrassLayer(geom, this);
	layerList.push_back(layer);
...
void GrassLoader::frameUpdate()
{
	...
	for (it = layerList.begin(); it != layerList.end(); ++it){
		...
		Technique* tech = layer->material->getTechnique(0);
...
void GrassLoader::loadPage(PageInfo &page)
{
	...
	for (it = layerList.begin(); it != layerList.end(); ++it){
		...
		if (grassCount != 0){
			...
			SceneManager::createMonoObjects = false;
			...
			SceneManager::createMonoObjects = true;
...
Mesh *GrassLoader::generateGrass_QUAD(PageInfo &page, GrassLayer *layer, const float *grassPositions, unsigned int grassCount)
{
	...
	MeshPtr mesh = MeshManager::getSingleton().createManual(getUniqueID(), "Assets");
	...
	for (uint16 i = 0; i < grassCount; ++i)
	{
		...
		Vector3 v0 = Vector3(float(x1 - page.centerPoint.x), float(y1 + scaleY), float(z1 - page.centerPoint.z));
		Vector3 v1 = Vector3(float(x2 - page.centerPoint.x), float(y2 + scaleY), float(z2 - page.centerPoint.z));
		Vector3 v2 = Vector3(float(x1 - page.centerPoint.x), float(y1), float(z1 - page.centerPoint.z));
	...
	subMesh->setMaterialName(layer->material->getName(), "Assets");
...
void GrassLayer::setMaterialName(const String &matName)
{
	if (!material || matName != material->getName()){
		material = Ogre::static_pointer_cast<Material>(MaterialManager::getSingleton().getByName(matName, "Assets"));
		if (!material)
...
void GrassLayer::_updateShaders()
{
	if (shaderNeedsUpdate){
		...
		if (caps->hasCapability(RSC_VERTEX_PROGRAM) && geom->getShadersEnabled())
		{
			...
			MaterialPtr tmpMat = MaterialManager::getSingleton().getByName(matName, "Assets");
			if (!tmpMat)
			{
				...
				for (int i = 0; i < tech->getNumPasses(); ++i)
				{
					...
					params->setNamedAutoConstant("world", GpuProgramParameters::ACT_WORLD_MATRIX);
			...
			GetEngine->UpdateShaders(material);
		}

		PagedGeometry* grassGeom = this->geom;

		if (grassGeom != nullptr)
		{
			auto lst = grassGeom->getDetailLevels();
			for (auto it = lst.begin(); it != lst.end(); ++it)
			{
				...
				for (auto _t = pages.begin(); _t != pages.end(); ++_t)
				{
					...
					if (dynamic_cast<GrassPage*>(page))
					{
						GrassPage* gp = (GrassPage*)page;
						auto nlst = gp->getNodeList();
						for (auto l = nlst.begin(); l != nlst.end(); ++l)
						{
							SceneNode* _nd = *l;
							Entity* ent = (Entity*)_nd->getAttachedObject(0);
							SubEntity* se = ent->getSubEntity(0);

							/*MaterialPtr oldMat = se->getMaterial();
							if (oldMat != nullptr)
							{
								if (MaterialManager::getSingleton().resourceExists(oldMat->getName(), oldMat->getGroup()))
									MaterialManager::getSingleton().remove(oldMat->getName(), oldMat->getGroup());
							}*/

							//se->setOriginalMaterial(material);
	...
	for (int j = 0; j < tech->getNumPasses(); ++j)
	{
		...
		GetEngine->UpdateShadowTextureUnits(pass);
		GetEngine->UpdateGpuProgramParams(pass);
	}
}
...
void GrassPage::removeEntities()
{
	...
	for (i = nodeList.begin(); i != nodeList.end(); ++i)
	{
		...
		for(int j = 0; j < numObjs; j++)
		{
			...
			if(!ent) continue;
			// remove the mesh
			MeshManager::getSingleton().remove(ent->getMesh()->getName(), "Assets");
			// then the entity
