...
float width = RenderedTextureWidth;
float height = RenderedTextureHeight;
...
float getDepth(vec2 coord){
	float zsample = texture2D(DepthTexture, coord).w;
	return -zfar * znear / (zsample * (zfar - znear) - zfar);
}

vec4 getColor(vec2 coord){
	return texture2D(RenderedTexture, coord);
}
...
vec2 pixelCoord = vec2(int(texCoord.x*width),int(texCoord.y*height));
vec3 fragPos = getViewPosition(texCoord);
vec3 fragNorm = getViewNormal(texCoord);
...
float fresnel(vec3 normal, vec3 incoming){
	float r = texture2D(MetallicTexture, texCoord).r;

	return r + (1.0-r) * pow(1.0-dot(normal, incoming), 5.0);
}

float brdf(vec3 normal, vec3 incoming){
	float k = roughness * roughness * 0.5;
	
	float geometry_A = dot(normal, fragView) / (dot(normal, fragView) * (1.0-k) + k);
	float geometry_B = dot(normal, incoming) / (dot(normal, incoming) * (1.0-k) + k);
...
vec3 raymarchRefine(vec3 position, vec3 direction){
	for(int i = 0; i < refineSteps; i++){
		direction *= 0.5;
		position += direction * (position.z < getDepth(getCoord(position)) ? 1.0 : -1.0);
	}
	return position;
}
...
void main()
{
	//vec4 direct = getColor(texCoord);
	vec4 reflection = glossyReflection();
	
	//gl_FragColor = (reflection + direct * (1.0-reflection.w));

	float r = texture2D(MetallicTexture, texCoord).r;
	
	gl_FragColor = reflection * r;
}