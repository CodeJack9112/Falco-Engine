...
void TreeNode::update(bool opened)
{
	if (popupMenuItems.size() > 0)
	{
		...
		{
			ImGui::SameLine();
			if (ImGui::Button(buttonText.c_str(), ImVec2(0, 19)))
			{
				_btn = true;
...
void TreeNode::genName(string name)
{
	boost::uuids::random_generator uuid_gen;
	boost::uuids::uuid u = uuid_gen();

	string uName = boost::uuids::to_string(u);

	this->alias = name;
	this->name = uName;
}

string TreeNode::genGuid()
{
	boost::uuids::random_generator uuid_gen;
	boost::uuids::uuid u = uuid_gen();
...
void TreeNode::setPopupMenu(std::vector<string> items, std::function<void(TreeNode * node, int val)> callback)
{
	popupMenuItems = items;
	onPopupSelectedCallback = callback;
}

bool TreeNode::isFormatSupported(string fmt, TreeView* tree)
{
