...
Real DensityMap::setDensityAt(Ogre::Real x, Ogre::Real z, const Ogre::TRect<Ogre::Real>& mapBounds, float value)
{
	assert(pixels);
	...
	data[mapWidth * zindex + xindex] = value * 255.0f;
}

Real DensityMap::setDensityAtRaw(Ogre::Real x, Ogre::Real z, const Ogre::TRect<Ogre::Real>& mapBounds, uint8 value)
{
	if (x < mapBounds.left || x >= mapBounds.right || z < mapBounds.top || z >= mapBounds.bottom)
		return 0.f;
	...
	uint8 * data = reinterpret_cast<uint8*>(pixels->data);
	data[mapWidth * zindex + xindex] = value;
}

uint8 DensityMap::getDensityAtRaw(Ogre::Real x, Ogre::Real z, const Ogre::TRect<Ogre::Real>& mapBounds)
{
	assert(pixels);
...
*/
	...
	uint8 *data = reinterpret_cast<uint8*>(pixels->data);
	return (float)data[mapWidth * zindex + xindex] / 255.0f;// *0.00392157f; // 1/255.0f;
}
