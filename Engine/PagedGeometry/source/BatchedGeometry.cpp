...
m_pParentGeom           (parent)
{
   ...
   updateMaterial();
      ...
      //GetEngine->UpdateShaders(m_ptrMaterial);
   //}
   //m_ptrMaterial = parentMaterial;
...
BatchedGeometry::SubBatch::~SubBatch()
{
    ...
    if (m_ptrMaterial != nullptr)
        MaterialManager::getSingleton().remove(m_ptrMaterial);
}
...
void BatchedGeometry::SubBatch::addSelfToRenderQueue(RenderQueueGroup* rqg)
{
	...
	if (m_ptrMaterial != nullptr)
	{
		...
		if (m_pBestTechnique != nullptr)
			rqg->addRenderable(this, m_pBestTechnique, OGRE_RENDERABLE_DEFAULT_PRIORITY);

        for (int i = 0; i < m_pBestTechnique->getNumPasses(); ++i)
        {
            Pass* pass = m_pBestTechnique->getPass(i);
...
void Forests::BatchedGeometry::SubBatch::updateMaterial()
{
    if (m_ptrMaterial != nullptr && m_ptrMaterial != subEntity->getOriginalMaterial())
        MaterialManager::getSingleton().remove(m_ptrMaterial->getName(), m_ptrMaterial->getGroup());

    MaterialPtr parentMaterial = subEntity->getOriginalMaterial();
    if (parentMaterial == nullptr)
        parentMaterial = subEntity->getMaterial();

    Ogre::String newName = parentMaterial->getName() + "_Batched";
    if (MaterialManager::getSingleton().resourceExists(newName, parentMaterial->getGroup()))
    {
